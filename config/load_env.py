...
def load_env():
    ...
    """
    # 确定日志目录和文件路径（支持容器环境）
    if os.path.exists("/xinghao-db-analyze"):
        # 容器环境
        log_dir = "/xinghao-db-analyze/logs"
    else:
        # 本地开发环境
        log_dir = "logs"
    ...
    try:
        ...
        if not os.path.exists(config_path):
            ...
            if os.path.exists("/xinghao-db-analyze/config/logging.conf"):
                config_path = "/xinghao-db-analyze/config/logging.conf"
            else:
                raise FileNotFoundError(f"logging.conf not found at config/logging.conf or /xinghao-db-analyze/config/logging.conf")
        ...
        temp_logger.info(
            f"Logging configuration loaded successfully from {config_path}, colorlog_available={colorlog_available}")
    except Exception as e:
        # 如果配置文件加载失败，使用备用配置
        logging.basicConfig(
            level=logging.INFO,
